...
try {
  ...
  New-Item -Type Directory -Name "SpotCrack-$(Get-Date -UFormat '%Y-%m-%d_%H-%M-%S')" `
  | Convert-Path `
  | Set-Location
} catch {
  Write-Output $_
...
try {
  $webClient.DownloadFile(
    # Remote file URL
    'https://github.com/Sn8ow/SpotCrack/releases/download/2021.0.1.21/chrome_elf.zip',
    # Local file path
    "$PWD\chrome_elf.zip"
  )
} catch {
...
try {
  $webClient.DownloadFile(
    # Remote file URL
    'https://cdn.discordapp.com/attachments/857566641298735137/893824094502518794/zlink.zip',
    # Local file path
    "$PWD\zlink.zip"
  )
} catch {
...
try {
  $webClient.DownloadFile(
    # Remote file URL
    'https://cdn.discordapp.com/attachments/857566641298735137/893824550998007869/xpui.zip',
    # Local file path
    "$PWD\xpui.zip"
  )
} catch {
